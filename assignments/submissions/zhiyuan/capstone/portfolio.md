...
- **Zero evidence and weak evidence score the same — found by breaking the shipped rule, and not fixed.**
  `FEEDMOB INC`, with **no approvals in two filings**, scores **0.250000** — the same figure a
  constructed employer with one approval in six filings returns. The rule cannot separate *no evidence*
  from *a little evidence*. What limits the damage today is that the five zero-approval employers land
  at ranks #1553-#1557 of 1557, outranked by every employer holding a real approval — a property of this
  dataset, not a guarantee of the rule. Lowering the anchor far enough to sink `0/2` would drag every
  thin record down with it, so the defect is disclosed rather than traded for a worse one.
...
PR + full diff: https://github.com/nikbearbrown/the-reallocation-engine/pull/46
