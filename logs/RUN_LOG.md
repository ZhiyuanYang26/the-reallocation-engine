...
- **Contribution:** `scripts/score/sponsorship-credibility.mjs` (`npm run score:sponsor-credibility`) — a sample-size-aware sponsorship credibility feed for the Ch.11 Role Scorer (Ch 5 + 11 + 16). Rule: `(approvals + 1) / (total + 2)` (Laplace's rule of succession, anchor 0.5).
...
- **Real result:** 1557 scored / 28812 EMPTY / 0 ERROR; 215 distinct scores. Self-test 10/10, exit 0. `--compare`: top-10 overlap **0/10**, median sample size in the list 10 -> 276 filings.
- **Method replaced after a break attempt (v1 -> v2):** v1 was beta-binomial empirical Bayes with the prior fitted at run time (alpha=17.0132, beta=0.3472, anchored at the 98.1% pooled rate). Feeding it adverse thin records broke it — FEEDMOB INC (0 approvals / 2 filings) scored 0.878762 and 25 employers were lifted >0.20 above their own rate. A first patch (refuse to promote adverse records; attach a caution string) was discarded as decoration — a true label wrapped around a wrong number. The anchor was moved to 0.5; FEEDMOB now scores 0.250, held by two new invariants (`c(0/2) > c(0/10) > c(0/40)`, `c(0/2) < 0.30`).
- **Residual risk logged:** 0.250 is not 0 — two filings cannot establish that an employer never sponsors. Gate G5 (`--movers`) requires a human to read the adjustments in both directions.
- **Known limits logged:** near-top gaps are thousandths (CONFLUENT 0.998366 vs JUNIPER 0.997596); among the 1262 perfect records the score reduces to n/(n+2), i.e. sample size alone, which favours employers that file often; the score is not a rate and `raw_rate` is emitted alongside.
